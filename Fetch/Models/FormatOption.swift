...
struct MediaInfo: Sendable {
    ...
    let hasAutoGeneratedCaptions: Bool
    ...
    init(json: [String: Any], url: String) {
        ...
        self.channelURL = json["channel_url"] as? String ?? json["uploader_url"] as? String
        self.channelFollowerCount = json["channel_follower_count"] as? Int
        self.tags = json["tags"] as? [String] ?? []
        self.categories = json["categories"] as? [String] ?? []
        self.webpageURL = json["webpage_url"] as? String

        // Parse subtitle languages from yt-dlp JSON
        let subtitles = json["subtitles"] as? [String: Any] ?? [:]
        let autoCaptions = json["automatic_captions"] as? [String: Any] ?? [:]

        // Merge manual and auto-caption language codes, deduped
        var languageSet = Set(subtitles.keys)
        languageSet.formUnion(autoCaptions.keys)
        self.subtitleLanguages = Array(languageSet)
        self.hasAutoGeneratedCaptions = !autoCaptions.isEmpty
